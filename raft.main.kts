    ...
    fun handleOperation(echoMsg: NodeMsg) {
        ...
        fun advanceStateMachine() {
            while (lastAppliedIndex < commitIndex) {
                ...
                val randMsgId = (0..10000).random()
                val replyBody = MsgBody(
                    resp.type,
                    msgId = randMsgId,
                    inReplyTo = logEntry?.body?.msgId,
                    value = resp.value,
                    code = resp.code
                )
            ...
            thread {
                ...
                neighBorIds.map {
                    ...
                    val replyBody = MsgBody(
                        type = "request_vote",
                        msgId = randMsgId,
                        term = termForVoteRequested,
                        candidateId = nodeId,
                        lastLogIndex = entriesLog.size,
                        lastLogTerm = entriesLog.last().term
                    )
                    ...
                    if (entriesToReplicate.size > 0 || heartBeatInterval < elapsedTime) {
                        ...
                        if (nodeState == "leader" && replyBody.term == term) {
                            ...
                            if (replyBody.success ?: false) {
                                nextIndexMap.put(
                                    neighBorId,
                                    maxOf(nextIndexMap.get(neighBorId) ?: 0, ni.plus(entriesToReplicate.size))
                                )
                                matchIndexMap.put(
                                    neighBorId,
                                    maxOf(matchIndexMap.get(neighBorId) ?: 0, ni.plus(entriesToReplicate.size - 1))
                                )
                                advanceCommitIndex()
                            } else {
